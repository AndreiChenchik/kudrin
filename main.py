...
newlink_filter = Filters.text([newlink]) & (Filters.user(user_id=POWER_USER_ID) | Filters.user(username=POWER_USER_NAME))
recalculate_filter = Filters.text([recalculate]) & (Filters.user(user_id=POWER_USER_ID) | Filters.user(username=POWER_USER_NAME))
...
def unknown(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="I'm sorry Dave I'm afraid I can't do that.", reply_markup=reply_markup)
...
def recalculate_balance(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id,text=f"*Budget status on {diff_date}*", parse_mode=ParseMode.MARKDOWN)
    ...
    context.bot.send_message(chat_id=update.effective_chat.id,text=f"Balance: *{diff:+.2f}₽*", parse_mode=ParseMode.MARKDOWN)
    context.bot.send_message(chat_id=update.effective_chat.id,text=f"Recommended daily budget: *{new_daily:.0f}₽*", parse_mode=ParseMode.MARKDOWN)
